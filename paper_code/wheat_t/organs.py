...
# ═══════════════════════════════════════════════════════════════════════
# OrganPool — single organ (v5: two-pool suc + str)
# ═══════════════════════════════════════════════════════════════════════

class OrganPool:
    """Carbon and OBT pools for a single plant organ (v44: two-pool, no OBT_fast)."""
    ...
    def step_metabolism(self, dt, is_night, TFWT_local, T_mean=25.0, is_sink=False, obt_signal=0.0,
                        is_exposed=False):
        """Internal metabolism: respiration + growth + dark OBT + resp OBT release (v44).

        v44: Removed NU_EXPOSE_* (black-box injection). All non-leaf organs get OBT
        ...
        # ── 3. OBT formation (v45: fast exposure + dark reaction + dynamic exchange) ──
        if self.name != 'grain' and self.W_str > 1e-20:
            ...
            if not is_exposed and TFWT_local > 0:
                nu = cfg.NU_DARK
                # v47: ear has independent dark metabolism (3× leaf, Diabaté Fig.2)
                if self.name == 'ear':
                    nu = cfg.NU_EAR_DARK
            ...
            # 3c. OBT_exch dynamic exchange (v45: replaces instantaneous equilibrium)
            ...
            # 3d. OBT_str decay (v45: non-leaf organs use own k_turn_day only)
            ...
            # v47: add non-leaf OBT_str decay (ds analysis: faster turnover for stem/ear)
...
# ═══════════════════════════════════════════════════════════════════════
# PlantOrgans — collection of all organs (v5: sink-strength allocation)
# ═══════════════════════════════════════════════════════════════════════
...
class PlantOrgans:
    ...
    def __init__(self, leaf_C_total, crop_name='wheat', grain_fill_max_h=None):
        ...
        self._fill_progress_at_exposure = -1.0  # v45r3: fill progress when exposure occurred (-1=not set)
        self._receptivity_at_exposure = 1.0     # v59: snapshot receptivity (fixed after exposure)
        ...
        self._grain_water_frac = 1.0  # v23: grain water fraction (DVS dependent)
        self._last_dW_grain_rate = 0.0  # v23: grain allocation rate for TFWT coupling
        self._last_dW_grain = 0.0  # grain carbon increment per step (for phloem loading)
        self._prev_grain_W_for_rate = 0.001  # v23: for growth rate coupling
        ...
        # v42: expH stored for ear dose correction
        self._expH = 2.0

    def update_organ_TFWT(self, C_air, C_slow_canopy, dt, dvs, canopy_mean_TFWT=0.0, C_xylem=0.0):
        """Update TFWT transport chain for all organs (§3.14.5, v44: + surface absorption).

        v44: Added surface absorption for stem/ear during exposure.
        ...
        # v44: Surface absorption during exposure — stem directly absorbs atmospheric HTO
        ...
        # v44: Surface absorption for ear during exposure
        ...
        # v26: grain water fraction decreases as grain matures (DVS 1.0→2.0)
        if dvs >= 1.0:
            ...
            if canopy_TFWT > self._grain_TFWT:  # exposure ongoing — HTO entering grain via phloem
                ...
                # v35: Scale rise by fill_progress — early grain fill has less phloem activity
    ...
    def compute_snapshot_receptivity(fill_progress):
        """Compute snapshot receptivity from fill_progress (v59).
    ...
    def step_allocate(self, dW_leaf, S_leaf_suc, dvs, T_mean, dt, is_night,
                      ...
                      is_exposed=False):
        """Allocate carbon from leaves to organs via sink strength (v44).

        dW_leaf: total photo carbon from canopy [kg C/m² ground]
        S_leaf_suc: OBT concentration in leaf suc pool [Bq/kg C]
        v44: grain OBT injection removed — all grain OBT via step_grain_obt + senescence.
        No dose_factor.
        """
        ...
        # v23: Track grain allocation rate for grain_TFWT coupling
        ...
        for organ, frac in alloc:
            dW = dW_leaf * frac
            if dW > 1e-20:
                # v43: grain receives carbon only (no OBT injection here)
    ...
    def _run_metabolism(self, dt, is_night, T_mean=25.0, obt_signal=0.0,
        ...
        v45: passes organ-specific TFWT (from transport chain) for fast HTO metabolism.
    ...
    def receive_leaf_realloc(self, dW, S_leaf, is_night, expH=2.0):
        ...
        v44: OBT transfers with carbon via S_leaf. No dose correction.
        Only active during grain fill.
        v59: OBT gated by snapshot receptivity.
        """
        if dW < 1e-20:
            return
    ...
    def step_stem_reallocation(self, dt, dvs):
        """Stem carbon reallocation to grain during grain fill (v44).

        Draws from BOTH suc and str pools. OBT transfers proportional to mean OBT.
        v44: Use config K_RECEP for receptivity. Removed OBT_fast.
        """
        ...
        if dW > 1e-20:
            ...
            # v45r1: Receptivity based on grain development stage (fill_progress)
            receptivity = self._grain_receptivity()
    ...
    def step_ear_reallocation(self, dt, dvs):
        ...
        if dW > 1e-20:
            ...
            if dW_grain > 1e-20 and self._grain_filling_active:
                ...
                # v44: no dose correction (removed EAR_DOSE_POWER)
    ...
    def step_ear_obt_to_grain(self, dt, dvs):
        """Ear OBT → grain transfer during grain fill (v44).
        ...
        v44: Removed OBT_fast. Simplified.
        """
    ...
    def trigger_grain_fill(self):
        """v58: Start grain filling (GDD-based trigger at anthesis).
    ...
    def step_grain_obt(self, dt, dvs, T_mean=25.0, grain_TFWT=0.0, is_night=False,
        ...
        # v58: Only accumulate grain fill hours when grain filling is active
        ...
        # v46: based on W_grain (continuous transfer), not dW (stops when grain stops growing)
        ...
        # v47: Stem→grain OBT backflow (late DAF: canopy OBT exhausted, stem reserves feed grain)
    ...
    def step_ear_photosynthesis(self, PAR_top, T_ear, dt):
        """Independent ear photosynthesis (v3: OBT via F_DARK['ear'])."""
        if PAR_top <= 0:
            return
