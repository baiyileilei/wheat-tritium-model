...
class CanopyModel:
    ...
    def __init__(self, LAI, n_layers, height, wind_top, crop_params):
        ...
        # Two-pool carbon model (v5: suc=mobile, str=structural) [kg C/m² leaf]
        ...
        self._OBT_released = [0.0] * n_layers  # v47: canopy protein turnover HTO release (for redistribution)
    ...
    def step_two_chamber_water(self, i, C_air, dt, T_vol, C_xylem=0.0):
        """Update two-chamber TFWT for layer i (§3.5.8, v3.2: + xylem source).
        ...
        # v3.2: + xylem source term — xylem water enters slow chamber during transpiration
    ...
    # ─────────────────────────────────────────────────────────────────
    # §3.5 (v7)  Single-chamber leaf water model
    # ─────────────────────────────────────────────────────────────────
    ...
    def step_carbon_flow(self, i, dt, is_night, A_umol, obt_signal=0.0,
        ...
        """Carbon flux + OBT formation for leaf layer i (v45).
        ...
        # v47: reset released OBT tracker for this step
        ...
        # --- OBT_exch dynamic exchange (v45: replaces instantaneous equilibrium) ---
        ...
        if self.W_str[i] > 1e-20 and self.OBT_str[i] > 0:
            ...
            if R_m > 1e-20 and self.W_str[i] > 1e-20:
                ...
                # v47: track released OBT for stem/ear redistribution
                self._OBT_released[i] += dOBT_release * cfg.CANOPY_OBT_RELEASE_FRAC

        # --- OBT_str decay (v45: leaf-only, K_PROT_TURN + OBT_RESP_DECAY_LEAF) ---
        ...
        if self.OBT_str[i] > 0:
            ...
            self.OBT_str[i] = max(0.0, self.OBT_str[i])
            # v47: track protein turnover OBT release for stem/ear redistribution
    ...
    def step_senescence(self, dt, t_hour, t_anthesis, grain_fill_h=1176.0,
                        exp_delay_h=0.0):
        """DVS-driven leaf senescence (v45: natural OBT decay, no K_SEN_DAF).

        v45: Removed K_SEN_DAF artificial decay. DAF non-monotonicity now produced by:
        ...
        # v45r3: Receptivity sigmoid based on grain developmental stage at exposure
        ...
        # v45: No K_SEN_DAF — canopy OBT decay is handled naturally by
        ...
        for i in range(self.N):
            ...
            # v46: Always scale when dLAI changes (even if dLAI is small)
            ...
            self.OBT_str[i] *= scale  # v46: scale OBT_str too!
    ...
    def get_weighted_exch_obt(self):
        """LAI-weighted mean exchangeable OBT [Bq/kg C]. v34: for phloem loading."""
        total_obt = 0.0
        total_lai = 0.0
        for i in range(self.N):
    ...
    def total_obt_released(self):
        ...
        v47: This HTO is available for stem/ear surface re-absorption.
    ...
    def shrink_leaf_realloc(self, dt, dvs):
        """Carbon reallocation from leaves to organs (v38: OBT removed).
