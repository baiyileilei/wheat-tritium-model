...
class SolvegModel:
    ...
    def __init__(self, params=None):
        ...
        # DAF dynamic exposure (v18): exposure starts when DVS reaches threshold
        ...
        self._grain_fill_triggered = False  # v58: grain fill trigger flag
    ...
    def _check_conservation(self, step_i):
        """Warn if carbon drops unexpectedly (v5: two-pool)."""
        C_now = sum(
    ...
    def step(self, t, dt):
        ...
        # v45r3: Record fill progress at exposure for grain receptivity
        if is_exposed and not getattr(self, '_exposure_recorded', False):
            ...
            # v59: Compute snapshot receptivity from fill_progress
            self.organs._receptivity_at_exposure = self.organs.compute_snapshot_receptivity(fp)
            # v57: Record delay from anthesis to exposure (hours)
        ...
        # v42: Soil HTO sink — open soil absorbs atmospheric HTO during exposure (dry deposition).
        ...
        self.organs._expH = self.expH  # v42: pass expH for ear dose correction
        ...
        for i in range(self.canopy.N):
            ...
            # Two-chamber leaf water (v12: fast room + slow room)
            self.canopy.step_two_chamber_water(i, self.C_air_local, dt, T_vol,
                                               C_xylem=C_xylem)

            # Carbon flow + OBT formation (v7: uses TFWT directly)
        ...
        # ════════ v47: Canopy→stem/ear OBT redistribution ════════
        ...
        # Grain fill OBT: v44 leaf OBT_str remobilization (no dose correction)
        ...
        # v44: no dose_factor — removed DOSE_POWER_GRAIN
        ...
        # v38: SEL_PHLOEM 路径移除 — OBT通过senescence+ear realloc路径转移
        # 旧的phloem loading路径与senescence重复，且没有receptivity调制

        # Leaf carbon reallocation (v38: S_leaf=0, OBT不通过碳再分配转移)
        # v41: if ENABLE_SENESCENCE_OBT_TRANSFER=False, zero S_leaf to prevent OBT transfer
        ...
        if self.DVS >= 1.0 and self._t_anthesis is None:
            ...
            # v45r2: Set exposure delay for grain receptivity bell curve
        ...
        # v19: Grain fill weight reverted to 1.0 — grain_TFWT pathway handles DAF dependence
        # v37: pass exp_delay_h for DAF decay correction in senescence transfer
        ...
        # ════════ 14. DVS update (v38: 封顶在 DVS_MATURE) ════════
    ...
    def run(self, progress_every=100):
        ...
        for step_i in range(n_steps):
            ...
            # v58: Capture harvest snapshot when grain fill ends
    ...
    def _harvest_summary(self):
        ...
        v58: Uses snapshot captured at grain fill end for leaf/stem/ear
        (to avoid post-fill decay artifacts).
        """
        canopy_obt = self.canopy.total_leaf_obt_inventory()
        peak_tfwt = self.results['peak_TFWT']

        # v58: Use snapshot if available (captured at grain fill end)
